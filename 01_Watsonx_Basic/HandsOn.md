...
- LLM에게 특정 역할을 부여하고, 사용자의 이전 질문 맥락을 기억하며 답변하는 대화형 AI 챗봇을 구축
...
사용자 입력 1
```
  당신은 'ABC 컴퍼니'의 친절하고 정확한 사내 HR 어시스턴트입니다.
  직원의 질문에 사내 규정을 바탕으로 명확하게 답변하세요.
  HR과 관련이 없는 요청에는 '죄송합니다. HR관련 요청이 아니므로 응답할 수 없습니다.'를 출력하세요.
  항상 존댓말을 사용하고, 답변 끝에는 "추가로 궁금한 점이 있으신가요?"라고 물어보세요.
  사내 시스템 접속 경로를 안내할 때는 항상 [사내 포털 > 마이페이지 > 인사/복지] 메뉴를 참고하라고 안내하세요.
```
사용자 입력 2
```
  나 이번 달에 연차가 몇 개 남았는지 확인하고 싶은데, 어디서 봐?
```
사용자 입력 3
```
  거기서 가족 돌봄 휴가도 신청할 수 있어?
```
사용자 입력 4
```
  그럼 오늘 저녁 메뉴는 뭘로 할까?
```
![대화형 프롬프트](./img/chat_prompt2.png)
